...
variable "env_id_mapping" {
  description = "What managed identity should we use for each environment"
  type        = map(any)
  default = {
    "sbx" = "dev", # sbx resources will use dev identity
    "dev" = "dev", # dev resources will use dev identity
    "uat" = "dev", # uat resources will use prd identity
    "prd" = "prd"  # prd resources will use prd identity
  }
}
